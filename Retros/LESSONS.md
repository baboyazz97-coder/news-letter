...
### CCR 네트워크 샌드박스는 Bash curl을 차단한다 #tooling #strategy
CCR(Claude Code Remote) 원격 트리거 환경은 네트워크 샌드박스가 적용되어 있어 Bash에서 외부 호스트로의 curl 요청이 모두 차단된다. RSS 수집, API 호출 등 외부 네트워크가 필요한 작업은 WebSearch 도구를 사용해야 한다. 로컬에서 잘 동작하던 curl 기반 수집이 CCR에서 실패하면 이 제약이 원인일 가능성이 높다.
